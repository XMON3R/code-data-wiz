...
export interface SQLParser {
  /**
   * Parses a SQL input string and returns an internal SQLDiagram representation.
   * 
   * @param {string} input - SQL code as a string.
   * @returns {SQLDiagram} - Parsed representation of the SQL tables and structure.
   */
 ...
 * It supports basic column definitions and common table-level constraints.
 */
class SimpleSQLParser implements SQLParser {
  /**
   * A set of valid SQL base data types for parsing and validation.
   */
  ...
  /**
   * Parses the input SQL and returns a diagram containing all parsed tables.
   * 
   * @param {string} input - SQL source string.
   * @returns {SQLDiagram} Parsed representation.
   */
  parse(input: string): SQLDiagram {
    ...
    statements.forEach((statement) => {
      ...
      if (table.columns.length > 0 || table.constraints) {
        ...
        // Handle empty tables like `CREATE TABLE Logs ();`
        diagram.tables.push(table);
      }
    });
   ...
   *
   * @param {string} line - Raw SQL line defining a column.
   * @param {SQLTable} table - The table to which the column belongs.
   */
  ...
  /**
   * Converts a SQL type string like "VARCHAR(255)" into a structured SQLDataType object.
   *
   * @param {string} typeStr - Raw SQL data type string.
   * @returns {SQLDataType} Parsed data type object.
   */
  ...
  /**
   * Parses a table-level constraint such as PRIMARY KEY, FOREIGN KEY, or UNIQUE.
   *
   * @param {string} line - Constraint definition line.
   * @param {SQLTable} table - The table the constraint belongs to.
   */
  private parseTableConstraint(line: string, table: SQLTable): void {
    ...
    if (foreignKeyMatch) {
      ...
      const referencedColumns = foreignKeyMatch[4]
        ? foreignKeyMatch[4].replace(/`/g, '').split(',').map(col => col.trim())
        : columns;
